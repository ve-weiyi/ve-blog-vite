...
export const read = async(reader: any, target: Ref<string> | Ref<any[]>): Promise<any> => {
  ...
  const streamMessage = jsons.map((jn) => jn.choices.map((choice: any) => choice.delta.content).join('')).join('')
