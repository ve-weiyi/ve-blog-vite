...
import type { EmptyResp, OauthLoginUrlResp, ResetPasswordReq, UserEmailReq, LoginReq, LoginResp, OauthLoginReq, EmptyReq } from "./types"
...
/** "第三方登录授权地址" */
export function oauthAuthorizeUrlApi(data?: OauthLoginReq): Promise<IApiResponseData<OauthLoginUrlResp>> {
  return request({
...
/** "发送忘记密码邮件" */
export function sendForgetEmailApi(data?: UserEmailReq): Promise<IApiResponseData<EmptyResp>> {
  return request({
...
/** "发送注册账号邮件" */
export function sendRegisterEmailApi(data?: UserEmailReq): Promise<IApiResponseData<EmptyResp>> {
  return request({
